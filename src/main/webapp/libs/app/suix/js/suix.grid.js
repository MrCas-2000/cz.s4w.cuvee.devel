(function (f, define) {
    ...
    (function ($, undefined) {
        var suix = window.suix, ui = suix.ui, proxy = $.proxy, extend = $.extend, grep = $.grep, map = $.map, inArray = $.inArray, ACTIVE = 'k-state-selected', ASC = 'asc', DESC = 'desc', CHANGE = 'change', INIT = 'init', OPEN = 'open', SELECT = 'select', POPUP = 'suixPopup', FILTERMENU = 'suixFilterMenu', MENU = 'suixMenu', NS = '.suixColumnMenu', Widget = ui.Widget;
        function trim(text) {
            return suix.trim(text).replace(/&nbsp;/gi, '');
        }
        ...
        function attrEquals(attrName, attrValue) {
            return '[' + suix.attr(attrName) + '=\'' + (attrValue || '').replace(/'/g, '"') + '\']';
        }
        ...
        function oldColumnOccurrences(renderedListElements, checkBoxes) {
            var indexAttr = suix.attr('index');
            var fieldAttr = suix.attr('field');
            var columnDict = {};
            var signature;
            var columCheckbox;
            var index;
        ...
        var ColumnMenu = Widget.extend({
            init: function (element, options) {
                ...
                that.field = element.attr(suix.attr('field'));
                that.title = element.attr(suix.attr('title'));
            ...
            _createMenu: function () {
                ...
                that.wrapper.html(suix.template(template)({
                    uid: suix.guid(),
                    ns: suix.ns,
            ...
            _createMobileMenu: function () {
                var that = this, options = that.options;
                var html = suix.template(mobileTemplate)({
                    ns: suix.ns,
                    field: that.field,
            ...
            _angularItems: function (action) {
                ...
                that.angular(action, function () {
                    var items = that.wrapper.find('.k-columns-item input[' + suix.attr('field') + ']').map(function () {
            ...
            _columns: function () {
                ...
                if (that.options.columns) {
                    ...
                    that.menu.bind(SELECT, function (e) {
                        var item = $(e.item), input, column, indexAttr = suix.attr('index'), columnIndexMap = {}, columnsCount = 0, columns = grep(leafColumns(that.owner.columns), function (col, idx) {
                                var result = true, title = trim(col.title || '');
                                if (col.menu === false || !col.field && !title.length) {
                                    result = false;
                                }
            ...
            _updateColumnsMenu: function () {
                ...
                var fieldAttr = suix.attr('field'), lockedAttr = suix.attr('locked'), columnIndexMap = {}, columnsCount = 0, indexAttr = suix.attr('index'), colIdx = 0, columnsInMenu = grep(leafColumns(this.owner.columns), function (col, idx) {
                        var result = true, title = trim(col.title || '');
                        if (col.menu === false || !col.field && !title.length) {
                            result = false;
                        }
                ...
                for (idx = 0, length = checkboxes.length; idx < length; idx++) {
                    ...
                    checked = false;
                    switchWidget = current.data('suixSwitch');
            ...
            _updateColumnsLockedState: function () {
                ...
                var fieldAttr = suix.attr('field');
                var lockedAttr = suix.attr('locked');
        ...
        var mobileTemplate = '<div data-#=ns#role="view" class="k-grid-column-menu">' + '<div data-#=ns#role="header" class="k-header">' + '<a href="\\#" class="k-header-cancel k-link" title="#=messages.cancel#" ' + 'aria-label="#=messages.cancel#"><span class="k-icon k-i-arrow-chevron-left"></span></a>' + '${messages.settings}' + '<a href="\\#" class="k-header-done k-link" title="#=messages.done#" ' + 'aria-label="#=messages.done#"><span class="k-icon k-i-check"></span></a>' + '</div>' + '<div class="k-column-menu">' + '<ul class="k-reset">' + '<li>' + '<span class="k-list-title">#=messages.column#: ${title}</span>' + '<ul class="k-listgroup k-listgroup-flush">' + '#if(sortable){#' + '<li id="#=suix.guid()#" class="k-item k-listgroup-item k-sort-asc"><span class="k-link"><span class="k-icon k-i-sort-asc-sm"></span><span class="k-item-title">${messages.sortAscending}</span></span></li>' + '<li id="#=suix.guid()#" class="k-item k-listgroup-item k-sort-desc"><span class="k-link"><span class="k-icon k-i-sort-desc-sm"></span><span class="k-item-title">${messages.sortDescending}</span></span></li>' + '#}#' + '#if(lockedColumns){#' + '<li id="#=suix.guid()#" class="k-item k-listgroup-item k-lock"><span class="k-link"><span class="k-icon k-i-lock"></span><span class="k-item-title">${messages.lock}</span></span></li>' + '<li id="#=suix.guid()#" class="k-item k-listgroup-item k-unlock"><span class="k-link"><span class="k-icon k-i-unlock"></span><span class="k-item-title">${messages.unlock}</span></span></li>' + '#}#' + '#if(filterable){#' + '<li id="#=suix.guid()#" class="k-item k-listgroup-item k-filter-item">' + '<span class="k-link k-filterable">' + '<span class="k-icon k-i-filter"></span>' + '<span class="k-item-title">${messages.filter}</span>' + '<span class="k-select"><span class="k-icon k-i-arrow-chevron-right"></span></span>' + '</span>' + '</li>' + '#}#' + '</ul>' + '</li>' + '#if(showColumns){#' + '<li class="k-columns-item"><span class="k-list-title">${messages.columnVisibility}</span>' + '<ul class="k-listgroup k-listgroup-flush">' + '#for (var idx = 0; idx < columns.length; idx++) {#' + '<li id="#=suix.guid()#" class="k-item k-listgroup-item">' + '<span class="k-listgroup-form-row">' + '<span class="k-listgroup-form-field-label k-item-title">' + '#=columns[idx].title#' + '</span>' + '<span class="k-listgroup-form-field-wrapper">' + '<input type="checkbox" title="#=columns[idx].title#" ' + ' data-#=ns#field="#=columns[idx].field.replace(/"/g,"&\\#34;")#"' + ' data-#=ns#index="#=columns[idx].index#"' + ' data-#=ns#locked="#=columns[idx].locked#" />' + '</span>' + '</span>' + '</li>' + '#}#' + '</ul>' + '</li>' + '#}#' + '<li class="k-item k-clear-wrap">' + '<span class="k-list-title">&nbsp;</span>' + '<ul class="k-listgroup k-listgroup-flush">' + '<li class="k-listgroup-item">' + '<span class="k-link k-label k-clear" title="#=messages.clear#" aria-label="#=messages.clear#">' + '#=messages.clear#' + '</span>' + '</li>' + '</ul>' + '</li>' + '</ul>' + '</div>' + '</div>';
        var MobileMenu = Widget.extend({
            ...
            _createCheckBoxes: function () {
                var that = this;
                that.element.find('.k-columns-item').find('[type=\'checkbox\']').suixSwitch({
                    messages: {
                        checked: '',
                        unchecked: ''
                    },
            ...
            _destroyCheckBoxes: function () {
                ...
                for (var i = 0; i < elements.length; i++) {
                    switchWidget = elements.eq(i).data('suixSwitch');
                    if (switchWidget) {
...
(function (f, define) {
    define('suix.columnsorter', ['suix.core'], f);
}(function () {
    var __meta__ = {
        id: 'columnsorter',
        name: 'Column Sorter',
    ...
    (function ($, undefined) {
        ...
        var ColumnSorter = Widget.extend({
            ...
            refresh: function (e) {
                ...
                var that = this, sort = that.dataSource.sort() || [], dir, table, leafCells, element = that.element, field = element.attr(suix.attr(FIELD)), descriptor = (that.dataSource._sortFields || {})[field], headerIndex, sortOrder;
                element.removeAttr(suix.attr(DIR));
                element.removeAttr(ARIASORT);
                if (descriptor) {
                    dir = descriptor.dir;
                    element.attr(suix.attr(DIR), dir);
                    sortOrder = descriptor.index;
                }
                if (element.is('th') && descriptor) {
                    table = getColsTable(element);
                    if (table) {
                        if (element.attr(suix.attr('index'))) {
            ...
            _click: function (e) {
                var that = this, element = that.element, field = element.attr(suix.attr(FIELD)), dir = element.attr(suix.attr(DIR)), options = that.options, compare = that.options.compare === null ? undefined : that.options.compare, sort = that.dataSource.sort() || [], idx, length;
                e.preventDefault();
                ...
                if (this.dataSource.options.endless) {
                    ...
                    element.closest('.k-grid').getSuixGrid()._endlessPageSize = that.dataSource.options.pageSize;
...
(function (f, define) {
    ...
    (function ($, undefined) {
        var suix = window.suix, ui = suix.ui, proxy = $.proxy, support = suix.support, AUTOCOMPLETEVALUE = support.browser.chrome ? 'disabled' : 'off', POPUP = 'suixPopup', INIT = 'init', OPEN = 'open', REFRESH = 'refresh', CHANGE = 'change', NS = '.suixFilterMenu', EQ = 'Is equal to', NEQ = 'Is not equal to', roles = {
                ...
                'number': 'number',
                'date': 'date'
            }, isFunction = suix.isFunction, Widget = ui.Widget;
        ...
        var defaultTemplate = '<div class="k-filter-menu-container">' + '<div class="k-filter-help-text">#=messages.info#</div>' + '<select title="#=messages.operator#" data-#=ns#bind="value: filters[0].operator" data-#=ns#role="dropdownlist">' + '#for(var op in operators){#' + '<option value="#=op#">#=operators[op]#</option>' + '#}#' + '</select>' + '#if(values){#' + '<select title="#=messages.value#" data-#=ns#bind="value:filters[0].value" data-#=ns#text-field="text" data-#=ns#value-field="value" data-#=ns#source=\'#=suix.stringify(values).replace(/\'/g,"&\\#39;")#\' data-#=ns#role="dropdownlist" data-#=ns#option-label="#=messages.selectValue#" data-#=ns#value-primitive="true">' + '</select>' + '#}else{#' + '<input title="#=messages.value#" data-#=ns#bind="value:filters[0].value" class="k-textbox" type="text" #=role ? "data-" + ns + "role=\'" + role + "\'" : ""# />' + '#}#' + '#if(extra){#' + '<select title="#=messages.logic#" class="k-filter-and" data-#=ns#bind="value: logic" data-#=ns#role="dropdownlist">' + '<option value="and">#=messages.and#</option>' + '<option value="or">#=messages.or#</option>' + '</select>' + '<select title="#=messages.additionalOperator#" data-#=ns#bind="value: filters[1].operator" data-#=ns#role="dropdownlist">' + '#for(var op in operators){#' + '<option value="#=op#">#=operators[op]#</option>' + '#}#' + '</select>' + '#if(values){#' + '<select title="#=messages.additionalValue#" data-#=ns#bind="value:filters[1].value" data-#=ns#text-field="text" data-#=ns#value-field="value" data-#=ns#source=\'#=suix.stringify(values).replace(/\'/g,"&\\#39;")#\' data-#=ns#role="dropdownlist" data-#=ns#option-label="#=messages.selectValue#" data-#=ns#value-primitive="true">' + '</select>' + '#}else{#' + '<input title="#=messages.additionalValue#" data-#=ns#bind="value: filters[1].value" class="k-textbox" type="text" #=role ? "data-" + ns + "role=\'" + role + "\'" : ""#/>' + '#}#' + '#}#' + '<div class="k-action-buttons">' + '<button type="submit" title="#=messages.filter#" class="k-button k-primary">#=messages.filter#</button>' + '<button type="reset" title="#=messages.clear#" class="k-button">#=messages.clear#</button>' + '</div>' + '</div>';
        ...
        var FilterMenu = Widget.extend({
            init: function (element, options) {
                ...
                that.field = options.field || element.attr(suix.attr('field'));
            ...
            _init: function () {
                ...
                that.form.find('[' + suix.attr('role') + '=numerictextbox]').removeClass('k-textbox').end().find('[' + suix.attr('role') + '=datetimepicker]').removeClass('k-textbox').end().find('[' + suix.attr('role') + '=timepicker]').removeClass('k-textbox').end().find('[' + suix.attr('role') + '=datepicker]').removeClass('k-textbox');
                that.refresh();
                that.trigger(INIT, {
                    field: that.field,
                    container: that.form
                });
                suix.cycleForm(that.form);
            },
            _createForm: function (role) {
                ...
                operators = operators[type] || options.operators[type];
                that.form = $('<form title="' + that.options.messages.title + '" class="k-filter-menu"/>').html(suix.template(type === 'boolean' ? hasCustomTemplate ? customBooleanTemplate : booleanTemplate : defaultTemplate)({
                    field: that.field,
            ...
            _createMobileForm: function (role) {
                var that = this, options = that.options, operators = that.operators || {}, filterMenuGuid = suix.guid(), type = that.type;
                operators = operators[type] || options.operators[type];
                that.form = $('<div />').html(suix.template(type === 'boolean' ? booleanMobileTemplate : defaultMobileTemplate)({
                    field: that.field,
            ...
            refresh: function () {
                ...
                that.filterModel = suix.observable({
                    logic: 'and',
                    filters: defaultFilters
                });
                if (that.form) {
                    suix.bind(that.form.children().first(), that.filterModel);
                }
            ...
            destroy: function () {
                ...
                Widget.fn.destroy.call(that);
                if (that.form) {
                    suix.unbind(that.form);
                    suix.destroy(that.form);
                    that.form.unbind(NS);
                    if (that.popup) {
                        that.popup.destroy();
                        that.popup = null;
            ...
            _activate: function () {
                this.form.find(':suixFocusable:first').focus();
                this.trigger(OPEN, {
                    field: this.field,
                    container: this.form
                });
        ...
        var multiCheckNS = '.suixFilterMultiCheck';
        ...
        function distinct(items, field) {
            var getter = suix.getter(field, true), result = [], index = 0, seen = {};
        ...
        var DataSource = suix.data.DataSource;
        ...
        var FilterMultiCheck = Widget.extend({
            init: function (element, options) {
                ...
                var field = this.field = this.options.field || this.element.attr(suix.attr('field'));
            ...
            _createForm: function () {
                ...
                if (!this._isMobile) {
                    ...
                    if (options.messages.selectedItemsFormat) {
                        html += '<div class=\'k-filter-selected-items\'>' + suix.format(options.messages.selectedItemsFormat, 0) + '</div>';
                    }
                ...
                if (this._isMobile) {
                    that.form = $('<div />').html(suix.template(multiCkeckMobileTemplate)({
                        field: that.field,
                        title: options.title || that.field,
                        ns: suix.ns,
                        messages: options.messages,
                    ...
                    if (!options.appendToElement) {
                        that.popup = that.form.suixPopup({
                            anchor: that._link,
                            open: proxy(that._open, that),
                            activate: proxy(that._activate, that),
                            close: function () {
            ...
            createCheckAllItem: function () {
                var options = this.options;
                var template = suix.template(options.itemTemplate({
                    field: 'all',
            ...
            updateCheckAllState: function () {
                if (this.options.messages.selectedItemsFormat) {
                    this.form.find('.k-filter-selected-items').text(suix.format(this.options.messages.selectedItemsFormat, this.container.find(':checked:not(.k-check-all)').length));
                }
                if (this.checkBoxAll) {
            ...
            createCheckBoxes: function () {
                ...
                var template = suix.template(options.itemTemplate(templateOptions));
                var itemsHtml = suix.render(template, data);
            ...
            options: {
                ...
                itemTemplate: function (options) {
                    ...
                    if (mobile) {
                        return '<li class=\'k-item k-listgroup-item\'>' + '<label class=\'k-label k-listgroup-form-row\'>' + '<span class=\'k-listgroup-form-field-label k-item-title \'>#:suix.format(\'' + (format ? format : '{0}') + '\', ' + field + ')#</span>' + '<span class="k-listgroup-form-field-wrapper">' + '<input type=\'checkbox\' value=\'#:suix.format(\'{0' + valueFormat + '}\',' + valueField + ')#\'/>' + '</span>' + '</label>' + '</li>';
                    }
                    return '<li class=\'k-item\'>' + '<label class=\'k-label\'>' + '<input type=\'checkbox\' value=\'#:suix.format(\'{0' + valueFormat + '}\',' + valueField + ')#\'/>' + '<span>#:suix.format(\'' + (format ? format : '{0}') + '\', ' + field + ')#</span>' + '</label>' + '</li>';
                },
                checkAll: true,
                search: false,
                ignoreCase: true,
...
(function (f, define) {
    ...
    (function ($, undefined) {
        var suix = window.suix, Widget = suix.ui.Widget, outerWidth = suix._outerWidth, suixAttr = suix.attr, extend = $.extend, each = $.each, proxy = $.proxy, isRtl = false, DIR = 'dir', FIELD = 'field', TITLE = 'title', ASCENDING = 'asc', DESCENDING = 'desc', GROUP_SORT = 'group-sort', NS = '.suixGroupable', CHANGE = 'change', indicatorTmpl = suix.template('<div class="k-group-indicator" data-#=data.ns#field="${data.field}" data-#=data.ns#title="${data.title || ""}" data-#=data.ns#dir="${data.dir || "asc"}">' + '<a href="\\#" class="k-link">' + '<span class="k-icon k-i-sort-${(data.dir || "asc") == "asc" ? "asc-sm" : "desc-sm"}" title="(sorted ${(data.dir || "asc") == "asc" ? "ascending": "descending"})"></span>' + '${data.title ? data.title: data.field}' + '</a>' + '<a class="k-button k-button-icon k-flat">' + '<span class="k-icon k-i-close"></span>' + '</a>' + '</div>', { useWithBlock: false }), hint = function (target) {
                var title = target.attr(suix.attr('title'));
                if (title) {
                    title = suix.htmlEncode(title);
                }
                return $('<div class="k-header k-group-clue k-drag-clue" />').html(title || target.attr(suix.attr('field'))).prepend('<span class="k-icon k-drag-status k-i-cancel"></span>');
            }, dropCue = $('<div class="k-grouping-dropclue"/>');
        var Groupable = Widget.extend({
            init: function (element, options) {
                var that = this, group = suix.guid(), intializePositions = proxy(that._intializePositions, that), draggable, horizontalCuePosition, dropCuePositions = that._dropCuePositions = [];
                Widget.fn.init.call(that, element, options);
                isRtl = suix.support.isRtl(element);
                horizontalCuePosition = isRtl ? 'right' : 'left';
                that.draggable = draggable = that.options.draggable || new suix.ui.Draggable(that.element, {
                    filter: that.options.draggableElements,
                    hint: hint,
                    group: group
                });
                that.groupContainer = $(that.options.groupContainer, that.element).suixDropTarget({
                    ...
                    drop: function (e) {
                        var targetElement = e.draggable.currentTarget, field = targetElement.attr(suix.attr('field')), title = targetElement.attr(suix.attr('title')), sourceIndicator = that.indicator(field), dropCuePositions = that._dropCuePositions, lastCuePosition = dropCuePositions[dropCuePositions.length - 1], position;
                        ...
                        if (lastCuePosition) {
                            position = that._dropCuePosition(suix.getOffset(dropCue).left + parseInt(lastCuePosition.element.css('marginLeft'), 10) * (isRtl ? -1 : 1) + parseInt(lastCuePosition.element.css('marginRight'), 10));
                    ...
                    var newDir = indicator.attr(suixAttr(DIR)) === ASCENDING ? DESCENDING : ASCENDING;
                    indicator.attr(suixAttr(DIR), newDir);
                    that._change();
                    e.preventDefault();
                });
                draggable.bind([
            ...
            refresh: function () {
                ...
                var fieldAttr = suixAttr(FIELD);
                var titleAttr = suixAttr(TITLE);
                var indicatorHtml;
            ...
            destroy: function () {
                ...
                if (that.groupContainer.data('suixDropTarget')) {
                    that.groupContainer.data('suixDropTarget').destroy();
                }
                if (that.groupContainer.data('suixDraggable')) {
                    that.groupContainer.data('suixDraggable').destroy();
                }
            ...
            indicator: function (field) {
                ...
                return $.grep(indicators, function (item) {
                    return $(item).attr(suix.attr('field')) === field;
                })[0];
            },
            ...
            aggregates: function () {
                ...
                return that.element.find(that.options.filter).map(function () {
                    var cell = $(this), aggregate = cell.attr(suix.attr('aggregates')), member = cell.attr(suix.attr('field'));
            ...
            _canDrag: function (element) {
                var field = element.attr(suix.attr('field'));
                return element.attr(suix.attr('groupable')) != 'false' && field && (element.hasClass('k-group-indicator') || !this.indicator(field));
            },
            ...
            _dragEnd: function (draggable) {
                var that = this, field = draggable.currentTarget.attr(suix.attr('field')), sourceIndicator = that.indicator(field);
...
(function (f, define) {
    ...
    (function ($, undefined) {
        var suix = window.suix, outerWidth = suix._outerWidth, outerHeight = suix._outerHeight, getOffset = suix.getOffset, Widget = suix.ui.Widget, CHANGE = 'change', KREORDERABLE = 'k-reorderable';
        ...
        var Reorderable = Widget.extend({
            init: function (element, options) {
                var that = this, draggable, group = suix.guid() + '-reorderable';
                Widget.fn.init.call(that, element, options);
                element = that.element.addClass(KREORDERABLE);
                options = that.options;
                that.draggable = draggable = options.draggable || new suix.ui.Draggable(element, {
                    group: group,
                    autoScroll: true,
                ...
                element.find(draggable.options.filter).suixDropTarget({
                    group: draggable.options.group,
                    dragenter: function (e) {
                        if (!that._draggable) {
                            return;
            ...
            destroy: function () {
                ...
                that.element.find(that.draggable.options.filter).each(function () {
                    var item = $(this);
                    if (item.data('suixDropTarget')) {
                        item.data('suixDropTarget').destroy();
                    }
                });
                if (that.draggable) {
                    that.draggable.destroy();
...
(function (f, define) {
    ...
    (function ($, undefined) {
        var suix = window.suix, roleSelector = suix.roleSelector, ui = suix.ui, Widget = ui.Widget, ViewEngine = suix.ViewEngine, View = suix.View, extend = $.extend, NAVIGATE = 'navigate', VIEW_SHOW = 'viewShow', SAME_VIEW_REQUESTED = 'sameViewRequested', OS = suix.support.mobileOS, SKIP_TRANSITION_ON_BACK_BUTTON = OS.ios && !OS.appMode && OS.flatVersion >= 700, BACK = '#:back';
        var DOT = '.';
        var classNames = {
            pane: 'k-pane',
        ...
        var Pane = Widget.extend({
            init: function (element, options) {
                ...
                this._historyNavigate = function (url) {
                    ...
                    that.historyCallback(url, suix.parseQueryStringParams(url));
                };
                this._historyReplace = function (url) {
                    var params = suix.parseQueryStringParams(url);
            ...
            closeActiveDialogs: function () {
                ...
                dialogs.each(function () {
                    suix.widgetInstance($(this), ui).close();
                });
            },
            ...
            _setPortraitWidth: function () {
                ...
                if (portraitWidth) {
                    width = suix.mobile.application.element.is(DOT + classNames.vertical) ? portraitWidth : 'auto';
        ...
        Pane.wrap = function (element, options) {
            if (!element.is(roleSelector('view'))) {
                element = element.wrap('<div data-' + suix.ns + 'role="view" data-stretch="true"></div>').parent();
            }
...
(function (f, define) {
    ...
    (function ($, undefined) {
        var suix = window.suix, ui = suix.ui, DataSource = suix.data.DataSource, ObservableObject = suix.data.ObservableObject, tbodySupportsInnerHtml = suix.support.tbodyInnerHtml, activeElement = suix._activeElement, Widget = ui.Widget, outerWidth = suix._outerWidth, outerHeight = suix._outerHeight, keys = suix.keys, isPlainObject = $.isPlainObject, extend = $.extend, map = $.map, grep = $.grep, isArray = $.isArray, inArray = $.inArray, push = Array.prototype.push, proxy = $.proxy, isFunction = suix.isFunction, isEmptyObject = $.isEmptyObject, contains = $.contains, math = Math, DOT = '.', PROGRESS = 'progress', ERROR = 'error', HIERARCHY_CELL_CLASS = 'k-hierarchy-cell', DATA_CELL = ':not(.k-group-cell):not([' + suix.attr('virtual') + ']):not(.k-hierarchy-cell:not(:has(.k-icon.k-i-collapse,.k-icon.k-i-expand))):visible', SELECTION_CELL_SELECTOR = 'tbody>tr:not(.k-grouping-row):not(.k-detail-row):not(.k-group-footer) > td:not(.k-group-cell):not(.k-hierarchy-cell)', NAVROW = 'tr:not(.k-footer-template):visible', NAVCELL = ':not(.k-group-cell):not(.k-detail-cell):not(.k-hierarchy-cell):visible', ITEMROW = 'tr:not(.k-grouping-row):not(.k-detail-row):not(.k-footer-template):not(.k-group-footer):visible', FIRSTITEMROW = ITEMROW + ':first', LASTITEMROW = ITEMROW + ':last', FIRSTNAVITEM = NAVROW + ':first>' + NAVCELL + ':first', HEADERCELLS = 'th.k-header:not(.k-group-cell):not(.k-hierarchy-cell)', NS = '.suixGrid', CONTENTRLOCKEDCONTAINER = 'k-grid-content-locked', GROUPCELLCLASS = 'k-group-cell', EDIT = 'edit', BEFOREEDIT = 'beforeEdit', SAVE = 'save', REMOVE = 'remove', DETAILINIT = 'detailInit', FILTERMENUINIT = 'filterMenuInit', COLUMNMENUINIT = 'columnMenuInit', FILTERMENUOPEN = 'filterMenuOpen', COLUMNMENUOPEN = 'columnMenuOpen', CELLCLOSE = 'cellClose', CHANGE = 'change', COLUMNHIDE = 'columnHide', COLUMNSHOW = 'columnShow', SAVECHANGES = 'saveChanges', DATABOUND = 'dataBound', DETAILEXPAND = 'detailExpand', DETAILCOLLAPSE = 'detailCollapse', ITEM_CHANGE = 'itemchange', PAGE = 'page', PAGING = 'paging', SCROLL = 'scroll', SYNC = 'sync', FOCUSED = 'k-state-focused', FOCUSABLE = ':suixFocusable', SELECTED = 'k-state-selected', CHECKBOX = 'k-checkbox', CHECKBOXINPUT = 'input[data-role=\'checkbox\'].' + CHECKBOX, NORECORDSCLASS = 'k-grid-norecords', LINK_CLASS = 'k-link', ICON_CLASS = 'k-icon', ORDER_CLASS = 'k-sort-order', HEADER_COLUMN_MENU_CLASS = 'k-header-column-menu', FILTER_MENU_CLASS = 'k-grid-filter', RESIZE = 'resize', COLUMNRESIZE = 'columnResize', COLUMNREORDER = 'columnReorder', COLUMNLOCK = 'columnLock', COLUMNUNLOCK = 'columnUnlock', NAVIGATE = 'navigate', CLICK = 'click', MOUSEDOWN = 'mousedown', HEIGHT = 'height', TABINDEX = 'tabIndex', FUNCTION = 'function', STRING = 'string', BOTTOM = 'bottom', CONTAINER_FOR = 'container-for', FIELD = 'field', INPUT = 'input', INCELL = 'incell', INLINE = 'inline', UNIQUE_ID = 'uid', MINCOLSPANVALUE = 1, COLSPAN = 'colSpan', OVERFLOW = 'overflow', HIDDEN = 'hidden', SORT = 'sort', GROUP_SORT = 'group-sort', DELETECONFIRM = 'Are you sure you want to delete this record?', NORECORDS = 'No records available.', CONFIRMDELETE = 'Delete', CANCELDELETE = 'Cancel', COLLAPSE = 'Collapse', EXPAND = 'Expand', ARIALABEL = 'aria-label', formatRegExp = /(\}|\#)/gi, templateHashRegExp = /#/gi, whitespaceRegExp = '[\\x20\\t\\r\\n\\f]', nonDataCellsRegExp = new RegExp('(^|' + whitespaceRegExp + ')' + '(k-group-cell|k-hierarchy-cell)' + '(' + whitespaceRegExp + '|$)'), filterRowRegExp = new RegExp('(^|' + whitespaceRegExp + ')' + '(k-filter-row)' + '(' + whitespaceRegExp + '|$)'), COMMANDBUTTONTMPL = '# if (iconClass) {#' + '<a role="button" class="k-button k-button-icontext #=className#" #=attr# href="\\#"><span class="#=iconClass#"></span>#=text#</a>' + '# } else { #' + '<a role="button" class="k-button k-button-icontext #=className#" #=attr# href="\\#">#=text#</a>' + '# } #', SELECTCOLUMNTMPL = '<input class="' + CHECKBOX + '" data-role="checkbox" aria-label="Select row" aria-checked="false" type="checkbox">', SELECTCOLUMNHEADERTMPL = '<input class="' + CHECKBOX + '" data-role="checkbox" aria-label="Select all rows" aria-checked="false" type="checkbox">', isRtl = false, browser = suix.support.browser, isIE7 = browser.msie && browser.version == 7, isIE8 = browser.msie && browser.version == 8;
        ...
        var VirtualScrollable = Widget.extend({
            ...
            wrap: function () {
                var that = this, scrollbar = suix.support.scrollbar() + 1, element = that.element, wrapper;
                element.css({
                    width: 'auto',
                    overflow: 'hidden'
                ...
                that._wrapper();
                if (suix.support.kineticScrollNeeded || suix.support.touch) {
                    that.wrapper.css('touch-action', 'none');
                    that.drag = new suix.UserEvents(that.wrapper, {
                        global: true,
                        allowSelection: true,
                        start: function (e) {
                            e.sender.capture();
            ...
            _wheelScroll: function (e) {
                ...
                var scrollbar = this.verticalScrollbar, scrollTop = scrollbar.scrollTop(), delta = suix.wheelDeltaY(e);
            ...
            _scrollWrapperOnColumnResize: function () {
                ...
                if (wrapper[0].scrollWidth > wrapper[0].clientWidth) {
                    if (!that._wrapperScrolled && initialScrollTop || that._isScrolledToBottom()) {
                        wrapper.scrollTop(initialScrollTop + suix.support.scrollbar());
            ...
            _page: function (skip, take, callback) {
                ...
                if (isGroupPaged && dataSource._groupRangeExists() || !isGroupPaged && dataSource.inRange(skip, take)) {
                    suix.ui.progress($(that.wrapper).parent(), true);
                    dataSource.range(skip, take, function () {
                        suix.ui.progress($(that.wrapper).parent(), false);
                        callback();
                        that.trigger(PAGE);
                    }, 'page');
                } else {
            ...
            repaintScrollbar: function (shouldScrollWrapper) {
                var that = this, html = '', maxHeight = that.options.maxScrollHeight, dataSource = that.dataSource, scrollbar = !suix.support.kineticScrollNeeded ? suix.support.scrollbar() : 0, wrapperElement = that.wrapper[0], totalHeight, idx, itemHeight;
        ...
        function attrEquals(attrName, attrValue) {
            return '[' + suix.attr(attrName) + '=' + attrValue + ']';
        }
        ...
        function columnMatchesMedia(column) {
            return column && (isUndefined(column.media) || !isUndefined(column.media) && suix.matchesMedia(column.media));
        }
        ...
        function updateCellIndex(thead, columns, offset) {
            ...
            for (var idx = 0, length = columns.length; idx < length; idx++) {
                ...
                cell.attr(suix.attr('index'), offset + idx);
            }
        ...
        function childColumnsCells(cell) {
            ...
            var colSpanAttr = suix.attr('colspan');
        ...
        function normalizeCols(table, visibleColumns, hasDetails, groups) {
            var colgroup = table.find('>colgroup'), width, cols = map(visibleColumns, function (column) {
                    ...
                    if (width && parseInt(width, 10) !== 0) {
                        return suix.format('<col style="width:{0}"/>', typeof width === STRING ? width : width + 'px');
                    }
                    return '<col />';
                });
        ...
        function formatGroupValue(value, format, columnValues, encoded) {
            ...
            return format ? suix.format(format, groupValue) : encoded === false ? groupValue : suix.htmlEncode(groupValue);
        }
        ...
        var Grid = suix.ui.DataBoundWidget.extend({
            ...
            destroy: function () {
                ...
                if (that.options.reorderable) {
                    that.wrapper.data('suixReorderable').destroy();
                }
            ...
            getOptions: function () {
                ...
                result.columns = suix.deepExtend([], this.columns);
                var dataSource = this.dataSource;
            ...
            setOptions: function (options) {
                ...
                suix.deepExtend(currentOptions, options);
                ...
                this.options = null;
                if (this._isMobile) {
                    var mobileWrapper = wrapper.closest(suix.roleSelector('pane')).parent();
            ...
            dataItems: function () {
                var dataItems = suix.ui.DataBoundWidget.fn.dataItems.call(this);
                if (this.lockedContent) {
            ...
            _destroyColumnAttachments: function () {
                ...
                that.thead.add(that.lockedHeader).find('th').each(function () {
                    var th = $(this), filterMenu = th.data('suixFilterMenu'), sortable = th.data('suixColumnSorter'), columnMenu = th.data('suixColumnMenu');
                    if (filterMenu) {
                        filterMenu.destroy();
                    }
                    if (sortable) {
            ...
            _createResizeHandle: function (container, th) {
                ...
                resizeHandle.off('dblclick' + NS).on('dblclick' + NS, function () {
                    that._autoFitLeafColumn(parseInt(th.attr(suix.attr('index')), 10));
                });
            },
            _positionColumnResizeHandle: function () {
                ...
                that.thead.add(lockedHead).on('mousemove' + NS, 'tr:not(.k-filter-row) > th', function (e) {
                    ...
                    if (th[0].hasAttribute(suix.attr(COLSPAN))) {
                        return;
                    }
            ...
            _positionColumnResizeHandleTouch: function () {
                var that = this, lockedHead = that.lockedHeader ? that.lockedHeader.find('thead:first') : $();
                that._resizeUserEvents = new suix.UserEvents(lockedHead.add(that.thead), {
            ...
            resizeColumn: function (column, columnWidth) {
                ...
                var options = this.options;
                var scrollbar = !suix.support.mobileOS ? suix.support.scrollbar() : 0;
            ...
            _resizable: function () {
                var that = this, options = that.options, container, columnStart, columnWidth, columnMinWidth, gridWidth, isMobile = this._isMobile, scrollbar = !suix.support.mobileOS ? suix.support.scrollbar() : 0, isLocked, col, th;
                    ...
                    that.resizable = new ui.Resizable(container.add(that.lockedHeader), {
                        ...
                        resizeend: function () {
                            ...
                            if (columnWidth != newWidth) {
                                ...
                                var index = th.attr(suix.attr('index'));
                                if (!index) {
            ...
            _draggable: function () {
                ...
                if (that.options.reorderable) {
                    ...
                    that._draggableInstance = that.wrapper.suixDraggable({
                        group: suix.guid(),
                        autoScroll: true,
                        ...
                        hint: function (target) {
                            ...
                            return $('<div class="k-header k-reorder-clue k-drag-clue" />').html(title || target.attr(suix.attr('field')) || target.text()).prepend('<span class="k-icon k-drag-status k-i-cancel"></span>');
                        }
                    }).data('suixDraggable');
                }
            },
            _reorderable: function () {
                var that = this;
                if (that.options.reorderable) {
                    if (that.wrapper.data('suixReorderable')) {
                        that.wrapper.data('suixReorderable').destroy();
                    }
                    that.wrapper.suixReorderable({
            ...
            _modelForContainer: function (container) {
                ...
                var id = container.attr(suix.attr('uid'));
                ...
                if (editable) {
                    ...
                    if (mode === 'incell') {
                        that.table.add(that.lockedTable).on('mousedown' + NS, NAVROW + '>' + NAVCELL, function (e) {
                            var target = $(e.target);
                            if (that._editMode() === 'incell' && target.hasClass('k-checkbox') && target.prev().attr(suix.attr('bind'))) {
                                e.preventDefault();
                            }
                        });
                        if (editable.update !== false) {
                            ...
                            that.editableUserEvents = new suix.UserEvents(that.wrapper, {
            ...
            editCell: function (cell) {
                ...
                if (model && isColumnEditable(column, model) && !column.command) {
                    ...
                    that.editable = cell.addClass('k-edit-cell').suixEditable({
                        fields: {
                            field: column.field,
            ...
            _adjustLockedHorizontalScrollBar: function () {
                ...
                var scrollbar = table[0].offsetWidth > content[0].clientWidth ? suix.support.scrollbar() : 0;
                ...
                if (that.editable) {
                    if (that._editMode() === 'popup' && !that._isMobile) {
                        if (that._editableIsClosing) {
                            that._editContainer.data('suixWindow').bind('deactivate', destroy);
                        } else {
                            that._editableIsClosing = true;
                            that._editContainer.data('suixWindow').bind('deactivate', destroy).close();
                        }
                    } else {
                        destroy();
                    }
            ...
            _displayCell: function (cell, column, dataItem) {
                var that = this, state = {
                        storage: {},
                        count: 0
                    }, settings = extend({}, suix.Template, that.options.templateSettings), tmpl = suix.template(that._cellTmpl(column, state), settings);
                if (state.count > 0) {
            ...
            editRow: function (row) {
                ...
                that.cancelRow();
                if (model) {
                    row = that.tbody.children('[' + suix.attr('uid') + '=' + model.uid + ']');
                    that._attachModelChange(model);
                    if (mode === 'popup') {
            ...
            _createPopupEditor: function (model) {
                var that = this;
                var html = '<div ' + suix.attr('uid') + '="' + model.uid + '" class="k-popup-edit-form"><' + (that._isMobile ? 'ul class="k-edit-form-container k-listgroup k-listgroup-flush">' : 'div class="k-edit-form-container">');
                var column;
                var command;
                var fields = [];
                var idx;
                ...
                var attr;
                var editMenuGuid = suix.guid();
                var editable = that.options.editable;
                var template = editable.template;
                var options = isPlainObject(editable) ? editable.window : {};
                var settings = extend({}, suix.Template, that.options.templateSettings);
                var state;
                if (that.trigger(BEFOREEDIT, { model: model })) {
                    return;
                }
                ...
                if (template) {
                    if (typeof template === STRING) {
                        template = suix.unescape(template);
                    }
                    html += suix.template(template, settings)(model);
                    for (idx = 0, length = columns.length; idx < length; idx++) {
                        column = columns[idx];
                        if (column.command) {
                        ...
                        if (!column.command) {
                            if (!that._isMobile) {
                                ...
                                if (isColumnEditable(column, model)) {
                                    ...
                                    html += '<div ' + suix.attr('container-for') + '="' + column.field + '" class="k-edit-field"></div>';
                                } else {
                                    state = {
                                        storage: {},
                                        count: 0
                                    };
                                    tmpl = suix.template(that._cellTmpl(column, state), settings);
                                    if (state.count > 0) {
                                        tmpl = proxy(tmpl, state.storage);
                                    }
                                ...
                                if (isColumnEditable(column, model)) {
                                    ...
                                    html += '<div class="k-listgroup-form-field-wrapper" id="' + column.field + '_' + editMenuGuid + '" ' + suix.attr('container-for') + '="' + column.field + '"></div>';
                                    html += '</label>';
                                } else {
                                    state = {
                                        storage: {},
                ...
                if (!that._isMobile) {
                    ...
                    html += '</div></div></div>';
                    container = that._editContainer = $(html).appendTo(that.wrapper).eq(0).suixWindow(extend({
                        modal: true,
                        resizable: false,
                        draggable: true,
                    ...
                    html += '</ul></div>';
                    that.editView = that.pane.append('<div data-' + suix.ns + 'role="view" class="k-grid-edit-form">' + '<div data-' + suix.ns + 'role="header" class="k-header">' + '<a href="\\#" class="k-header-cancel k-link k-grid-cancel" title="#=messages.cancel#" ' + 'aria-label="#=messages.cancel#"><span class="k-icon k-i-arrow-chevron-left"></span></a>' + (that.options.messages.commands.edit || 'Edit') + '<a href="\\#" class="k-header-done k-link k-grid-update" title="#=messages.done#" ' + 'aria-label="#=messages.done#"><span class="k-icon k-i-check"></span></a>' + '</div>' + '<div data-' + suix.ns + 'role="content" class="' + classNames.content + '">' + html + '</div>' + '</div>');
                    container = that._editContainer = that.editView.element.find('.k-popup-edit-form');
                }
                that.editable = that._editContainer.suixEditable({
                    fields: fields,
                    model: model,
            ...
            _openPopUpEditor: function () {
                var that = this;
                var windowEditor = that._editContainer ? that._editContainer.data('suixWindow') : null;
            ...
            _createInlineEditor: function (row, model) {
                ...
                row.children(':not(.k-group-cell,.k-hierarchy-cell)').each(function () {
                    ...
                    if (!column.command && isColumnEditable(column, model)) {
                        ...
                        cell.attr(suix.attr('container-for'), column.field);
                        cell.empty();
                    } else if (column.command) {
                ...
                that.editable = new suix.ui.Editable(that._editContainer, {
                    target: that,
                    fields: fields,
                    model: model,
            ...
            cancelRow: function (notify) {
                ...
                if (container) {
                    ...
                    if (that._editMode() !== 'popup') {
                        ...
                        that._displayRow(that.tbody.find('[' + suix.attr('uid') + '=' + model.uid + ']'));
                    }
                }
            },
            saveRow: function () {
            ...
            _showMessage: function (messages, row) {
                ...
                var confirmDialog = that._confirmDialog = new suix.ui.Confirm($('<div />').appendTo(document.body), {
            ...
            _editModel: function (model) {
                ...
                var mode = that._editMode();
                if (model) {
                    var id = model.uid, table = that.lockedContent ? that.lockedTable : that.table, row = table.find('tr[' + suix.attr('uid') + '=' + id + ']'), cell = row.children('td:not(.k-group-cell,.k-hierarchy-cell)').eq(that._firstEditableColumnIndex(row));
                ...
                if (toolbar) {
                    ...
                    if (!container.length) {
                        if (!isFunction(toolbar)) {
                            ...
                            toolbar = proxy(suix.template(toolbar), that);
                        }
            ...
            _createButton: function (command) {
                ...
                return suix.template(template)(options);
            },
            ...
            _attachGroupable: function () {
                var that = this, wrapper = that.wrapper, groupable = that.options.groupable, draggables = HEADERCELLS + '[' + suix.attr('field') + ']', filter = that.content ? '.k-grid-header:first ' + draggables : 'table:first>.k-grid-header ' + draggables;
            ...
            _addGroupableOptionsToHeader: function () {
                ...
                for (i = 0; i < headerCells.length; i++) {
                    ...
                    cellFieldAttr = headerCell.attr(suix.attr(FIELD));
            ...
            _destroyGroupable: function () {
                ...
                if (that.groupable && that.groupable.element) {
                    that.groupable.element.suixGroupable('destroy');
                }
                that.groupable = null;
            ...
            _selectable: function () {
                ...
                if (selectable) {
                    ...
                    that._selectedIds = {};
                    selectable = suix.ui.Selectable.parseOptions(selectable);
                    ...
                    that.selectable = new suix.ui.Selectable(elements, {
                        filter: filter,
                        aria: true,
                        multiple: multi,
                        change: function () {
            ...
            select: function (items) {
                var that = this, selectable = that.selectable, selectableoptions = suix.ui.Selectable.parseOptions(this.options.selectable), cell = selectableoptions.cell;
            ...
            _scrollTo: function (element, container) {
                ...
                if (isRtl && isHorizontal) {
                    if (browser.msie || browser.edge) {
                        ...
                        firefoxCorrection = table.offsetLeft - suix.support.scrollbar();
                    }
                }
            ...
            _navigatable: function () {
                ...
                tables.on(suix.support.touch ? 'touchstart' + NS : 'mousedown' + NS, NAVROW + '>' + NAVCELL, proxy(tableClick, that)).on('focus' + NS, proxy(that._tableFocus, that)).on('focusout' + NS, proxy(that._tableBlur, that)).on('keydown' + NS, that, proxy(that._tableKeyDown, that));
            },
            ...
            _moveColumn: function (current, isLeft) {
                var elements = this.wrapper.data().suixReorderable.element.find(this._draggableInstance.options.filter + ':visible');
            ...
            _handleHome: function (current, ctrl) {
                ...
                var prev;
                if (this._hasVirtualColumns()) {
                    this._scrollToColumn(suix.keys.HOME);
                    return true;
                }
                if (ctrl) {
                    if (this.lockedTable) {
            ...
            _handleEnd: function (current, ctrl) {
                ...
                var next;
                if (this._hasVirtualColumns()) {
                    this._scrollToColumn(suix.keys.END);
                    return true;
                }
                if (ctrl) {
            ...
            _handleSpaceKey: function (current, ctrlKey) {
                ...
                var field = current.attr(suix.attr('field'));
                var aggregates = that.groupable.aggregates();
                var label = current.attr(suix.attr('title')) || field;
            ...
            _handleEnterKey: function (current, currentTable, target) {
                ...
                var focusable = current.find(':suixFocusable:first');
            ...
            _handleEditing: function (current, next, table) {
                ...
                if (that.editable) {
                    ...
                    if (that.editable.end()) {
                        ...
                        focusable = editContainer.find(':suixFocusable:first')[0];
                        if (focusable) {
                            focusable.focus();
                        }
                        return;
            ...
            _initMobile: function () {
                var options = this.options;
                var that = this;
                this._isMobile = options.mobile === true && suix.support.mobileOS || options.mobile === 'phone' || options.mobile === 'tablet';
                if (this._isMobile) {
                    var html = this.wrapper.addClass('k-grid-mobile').wrap('<div data-' + suix.ns + 'stretch="true" data-' + suix.ns + 'role="view" ' + 'data-' + suix.ns + 'init-widgets="false"></div>').parent();
            ...
            _scrollable: function () {
                ...
                if (scrollable) {
                    ...
                    if (virtualScroll.rows) {
                        ...
                        var touchScroller = that.content.data('suixTouchScroller');
                        if (touchScroller) {
                            touchScroller.destroy();
                        }
                        touchScroller = suix.touchScroller(that.content);
            ...
            _cacheEditableState: function () {
                ...
                if ((inlineMode || incellMode || virtualColumns) && editedModel) {
                    ...
                    if (editContainer && active[0] && contains(editContainer[0], active[0])) {
                        active.change();
                        widget = suix.widgetInstance(active, suix.ui);
                    ...
                    if (that._editMode() === INLINE) {
                        that._editableState = {
                            model: editedModel,
                            field: active.closest('[' + suix.attr(CONTAINER_FOR) + ']').attr(suix.attr(CONTAINER_FOR))
                        };
                    } else if (that._editMode() === INCELL) {
                        that._editableState = {
                            model: editedModel,
                            field: editContainer.attr(suix.attr(FIELD))
                        };
                    }
                }
            },
            ...
            _applyLockedContainersWidth: function () {
                if (this.options.scrollable && this.lockedHeader) {
                    var headerTable = this.thead.parent(), headerWrap = headerTable.parent(), contentWidth = this.wrapper[0].clientWidth, groups = this._groups(), scrollbar = suix.support.scrollbar(), cols = this.lockedHeader.find('>table>colgroup>col:not(.k-group-col, .k-hierarchy-col)'), nonLockedCols = headerTable.find('>colgroup>col:not(.k-group-col, .k-hierarchy-col)'), width = columnsWidth(cols), nonLockedColsWidth = columnsWidth(nonLockedCols), footerWrap;
                    if (groups > 0) {
            ...
            _setContentHeight: function () {
                var that = this, options = that.options, height, header = that.wrapper.children('.k-grid-header'), scrollbar = suix.support.scrollbar();
            ...
            _modelChange: function (e) {
                var that = this, tbody = that.tbody, model = e.model, row = that.tbody.find('tr[' + suix.attr('uid') + '=' + model.uid + ']'), relatedRow, cell, column, isAlt = row.hasClass('k-alt'), tmp, idx = that._items(tbody).index(row), isLocked = that.lockedContent, selectable, selectableRow, childCells, originalCells, length;
                if (isLocked) {
            ...
            _pageable: function () {
                ...
                if (pageable) {
                    ...
                    if (typeof pageable === 'object' && pageable instanceof suix.ui.Pager) {
                        that.pager = pageable;
                    } else {
                        if (that.dataSource._groupPaging) {
                            that.pager = new GroupsPager(pagerWrap, extend({}, pageable, { dataSource: that.dataSource }));
                        } else {
                            that.pager = new suix.ui.Pager(pagerWrap, extend({}, pageable, { dataSource: that.dataSource }));
                        }
                    }
            ...
            _wrapFooter: function (footerRow) {
                var that = this, html = '', scrollbar = !suix.support.mobileOS ? suix.support.scrollbar() : 0;
                if (that.options.scrollable) {
            ...
            _columnMenu: function () {
                ...
                if (columnMenu) {
                    ...
                    for (var idx = 0, length = cells.length; idx < length; idx++) {
                        ...
                        if (!column.command && (column.field || cell.attr('data-' + suix.ns + 'field'))) {
                            menu = cell.data('suixColumnMenu');
                            if (menu) {
                                menu.destroy();
                            }
                            ...
                            cell.suixColumnMenu(menuOptions);
                        }
                    }
                }
            },
            ...
            _filterable: function () {
                ...
                if (filterable && !that.options.columnMenu) {
                    ...
                    for (var idx = 0, length = cells.length; idx < length; idx++) {
                        cell = cells.eq(idx);
                        if (columns[idx].filterable !== false && !columns[idx].command && (columns[idx].field || cell.attr('data-' + suix.ns + 'field'))) {
                            filterMenu = cell.data('suixFilterMenu');
                            if (filterMenu) {
                                filterMenu.destroy();
                            }
                            filterMenu = cell.data('suixFilterMultiCheck');
                            if (filterMenu) {
                                filterMenu.destroy();
                            }
                            ...
                            if (columnFilterable && columnFilterable.multi) {
                                cell.suixFilterMultiCheck(options);
                            } else {
                                cell.suixFilterMenu(options);
                            }
                        }
                    }
                }
            ...
            _filterRow: function () {
                ...
                for (var i = 0; i < columns.length; i++) {
                    ...
                    if (field && col.filterable !== false) {
                        ...
                        $('<span/>').attr(suix.attr('field'), field).appendTo(th).suixFilterCell(settings);
                    } else {
                        th.html('&nbsp;');
                    }
                }
                ...
                if (sortable) {
                    ...
                    for (var idx = 0, length = cells.length; idx < length; idx++) {
                        ...
                        if (column.sortable !== false && !column.command && column.field) {
                            cell = cells.eq(idx);
                            sorterInstance = cell.data('suixColumnSorter');
                            if (sorterInstance) {
                                sorterInstance.destroy();
                            }
                            cell.attr('data-' + suix.ns + 'field', column.field).suixColumnSorter(extend({}, sortable, column.sortable, {
                                dataSource: that.dataSource,
                                aria: true,
            ...
            _columns: function (columns) {
                ...
                columns = columns.length ? columns : map(table.find('th'), function (th, idx) {
                    th = $(th);
                    var sortable = th.attr(suix.attr('sortable')), filterable = th.attr(suix.attr('filterable')), type = th.attr(suix.attr('type')), groupable = th.attr(suix.attr('groupable')), field = th.attr(suix.attr('field')), title = th.attr(suix.attr('title')), menu = th.attr(suix.attr('menu'));
                    if (!field) {
                    ...
                    return {
                        ...
                        menu: menu,
                        template: th.attr(suix.attr('template')),
            ...
            _headerCheckboxClick: function (e) {
                var that = this, checkBox = $(e.target), checked = checkBox.prop('checked'), parentGrid = checkBox.closest('.k-grid.k-widget').getSuixGrid();
                if (that !== parentGrid) {
                    return;
                }
                if (checked) {
            ...
            _checkboxClick: function (e) {
                ...
                if (that !== row.closest('.k-grid.k-widget').getSuixGrid()) {
                    return;
                }
                if (isSelecting) {
                    that.select(row);
            ...
            _tmpl: function (rowTemplate, columns, alt, skipGroupCells) {
                var that = this, settings = extend({}, suix.Template, that.options.templateSettings), paramName = settings.paramName, idx, length = columns.length, template, state = {
                        storage: {},
                        count: 0
                ...
                if (!rowTemplate) {
                    ...
                    if (length) {
                        rowTemplate += ' ' + suix.attr('uid') + '="#=' + suix.expr('uid', settings.paramName) + '#"';
                    }
                ...
                rowTemplate = suix.template(rowTemplate, settings);
                if (state.count > 0) {
            ...
            _dirtyCellTemplate: function (field, paramName) {
                var dirtyField;
                if (field && paramName) {
                    dirtyField = field.charAt(0) === '[' ? suix.expr(field, paramName + '.dirtyFields') : paramName + '.dirtyFields[\'' + field + '\']';
                    return '#= ' + paramName + ' && ' + paramName + '.dirty && ' + paramName + '.dirtyFields && ' + dirtyField + ' ? \' k-dirty-cell\' : \'\' #';
                }
                return '';
            },
            _headerCellText: function (column) {
                var that = this, settings = extend({}, suix.Template, that.options.templateSettings), template = column.headerTemplate, type = typeof template, text = column.title || column.field || '';
                if (type === FUNCTION) {
                    text = suix.template(template, settings)({});
                } else if (type === STRING) {
                    text = template;
                }
                return text;
            ...
            _cellTmpl: function (column, state) {
                var that = this, settings = extend({}, suix.Template, that.options.templateSettings), template = column.template, paramName = settings.paramName, field = column.field, html = '', idx, length, format = column.format, type = typeof template, columnValues = column.values;
                if (column.command) {
                    if (isArray(column.command)) {
                        for (idx = 0, length = column.command.length; idx < length; idx++) {
                            if (column.command[idx].visible) {
                                html += suix.format('#= {0}(data)? \'{1}\':\'\' #', column.command[idx].visible, that._createButton(column.command[idx]).replace(templateHashRegExp, '\\#').replace(/'/gi, '\\\''));
                            } else {
                ...
                if (type === FUNCTION) {
                    ...
                    html += '#var v =' + suix.stringify(convertToObject(columnValues)).replace(templateHashRegExp, '\\#') + '#';
                    html += '#var f = v[';
                    ...
                    html += column.encoded ? '#:' : '#=';
                    if (format) {
                        html += 'suix.format("' + format.replace(formatRegExp, '\\$1') + '",';
                    }
                    if (field) {
                        field = suix.expr(field, paramName);
            ...
            _noRecordsTmpl: function () {
                ...
                var settings = $.extend({}, suix.Template, this.options.templateSettings);
                ...
                if (this.options.noRecords.template) {
                    ...
                    template = suix.format(defaultTemplate, this.options.messages.noRecords, scrollableNoGridHeightStyles);
                }
                type = typeof template;
                if (type === 'function') {
                    state.storage['tmpl' + state.count] = template;
                ...
                tmpl = suix.template(suix.format(wrapper, NORECORDSCLASS, html), settings);
                if (state.count > 0) {
                    tmpl = $.proxy(tmpl, state.storage);
                }
                return tmpl;
            ...
            _footerTmpl: function (columns, aggregates, templateName, rowClass, skipGroupCells) {
                var that = this, settings = extend({}, suix.Template, that.options.templateSettings), paramName = settings.paramName, html = '', idx, length, template, type, storage = {}, count = 0, scope = {}, groups = that._groups(), fieldsMap = that.dataSource._emptyAggregates(aggregates), column;
                ...
                for (idx = 0, length = columns.length; idx < length; idx++) {
                    ...
                    if (template) {
                        if (type !== FUNCTION) {
                            ...
                            template = suix.template(template, scope);
                        }
                ...
                html += '</tr>';
                html = suix.template(html, settings);
                if (count > 0) {
            ...
            _groupHeaderTmpl: function (columns, aggregates, templateName, rowClass, skipGroupCells, groupHeaderColumnTemplateColumns) {
                ...
                if (headerTemplateIndex < MINCOLSPANVALUE && groupHeaderColumnTemplateColumns.length <= 1 && !skipGroupCells) {
                    html += !skipGroupCells ? groupCellBuilder(columns.length) : '';
                    return suix.template(html, settings);
                }
            ...
            _detailTmpl: function (template) {
                var that = this, html = '', settings = extend({}, suix.Template, that.options.templateSettings), paramName = settings.paramName, templateFunctionStorage = {}, templateFunctionCount = 0, groups = that._groups(), colspan = visibleColumns(leafColumns(that.columns)).length, type = typeof template;
            ...
            _createHeaderCells: function (columns, rowSpan) {
                ...
                for (idx = 0, length = columns.length; idx < length; idx++) {
                    ...
                    if (th.selectable) {
                        ...
                        if (index > -1) {
                            html += suix.attr('index') + '=\'' + index + '\'';
                        }
                        text = th.headerTemplate ? text : suix.template(SELECTCOLUMNHEADERTMPL)({});
                        html += '>' + text + '</th>';
                    } else if (th.command) {
                        html += '<th scope=\'col\'' + stringifyAttributes(th.headerAttributes);
                        if (rowSpan && !columns[idx].colSpan) {
                        ...
                        if (th.field) {
                            field = suix.attr('field') + '=\'' + th.field + '\' ';
                        }
                        ...
                        if (columns[idx].colSpan > 1) {
                            ...
                            html += suix.attr('colspan') + '=\'' + columns[idx].colSpan + '\'';
                        } else if (columns[idx].colSpan === 1) {
                            html += suix.attr('colspan') + '=\'' + columns[idx].colSpan + '\'';
                        }
                        if (th.title) {
                            title = th.title.replace('"', '&quot;').replace(/'/g, '\'');
                            html += suix.attr('title') + '="' + title + '" ';
                        }
                        if (th.groupable !== undefined) {
                            html += suix.attr('groupable') + '=\'' + th.groupable + '\' ';
                        }
                        ...
                        if (th.aggregates && th.aggregates.length) {
                            html += suix.attr('aggregates') + '=\'' + th.aggregates + '\'';
                        }
                        if (index > -1) {
                            html += suix.attr('index') + '=\'' + index + '\'';
                        }
            ...
            _thead: function () {
                ...
                if (hasFilterRow) {
                    ...
                    if (existingFilterRow.length) {
                        suix.destroy(existingFilterRow);
                ...
                if (hasLockedColumns) {
                    ...
                    if (suix.support.touch) {
                        that._lockedContentUserEvents = new suix.UserEvents(that.lockedContent, {
                            move: function (e) {
            ...
            _wheelScroll: function (e) {
                ...
                var scrollTop = content.scrollTop(), delta = suix.wheelDeltaY(e);
                if (delta) {
            ...
            _groupRowHtml: function (group, colspan, level, groupHeaderBuilder, templates, skipColspan, skipLastGroup) {
                ...
                if (template && !skipColspan) {
                    text = typeof template === FUNCTION ? template(groupData) : suix.template(template)(groupData);
                }
            ...
            _setParentsVisibility: function (column, visible) {
                ...
                if (columnParents(column, columns, parents) && parents.length) {
                    for (idx = parents.length - 1; idx >= 0; idx--) {
                        ...
                        if (cell.filter('[' + suix.attr('colspan') + ']').length) {
                            colSpan = parseInt(cell.attr(suix.attr('colspan')), 10);
            ...
            _progress: function (toggle) {
                ...
                if (endless && toggle) {
                    suix.ui.progress(element, toggle, {
            ...
            refresh: function (e) {
                ...
                if (!that._endlessFetchInProgress) {
                    if (navigatable && (that._isActiveInTable() || that._editContainer && that._editContainer.data('suixWindow'))) {
                ...
                if (that.options.persistSelection && (that.selectable && !suix.ui.Selectable.parseOptions(that.options.selectable).cell || that._checkBoxSelection) && that.items().length) {
            ...
            _angularItems: function (cmd) {
                suix.ui.DataBoundWidget.fn._angularItems.call(this, cmd);
            ...
            _renderGroupRows: function () {
                ...
                groupRows.each(function (index, row) {
                    ...
                    if (template) {
                        text = typeof template === FUNCTION ? template(groupHeaderData) : suix.template(template)(groupHeaderData);
                    }
        ...
        if (suix.ExcelMixin) {
            suix.ExcelMixin.extend(Grid.prototype);
        }
        if (suix.PDFMixin) {
            suix.PDFMixin.extend(Grid.prototype);
            Grid.prototype._drawPDF_autoPageBreak = function (progress) {
                ...
                function draw() {
                    ...
                    suix.drawing.drawDOM(clone, options).always(function () {
                        cont.remove();
                    }).then(function (group) {
            ...
            Grid.prototype._drawPDF = function (progress) {
                ...
                var allPages = grid.options.pdf.allPages;
                this._initPDFProgress(progress);
                var doc = new suix.drawing.Group();
                var startingPage = dataSource.page();
                function resolve() {
                    if (allPages && startingPage !== undefined) {
            ...
            Grid.prototype._initPDFProgress = function (deferred) {
                ...
                var pb = $('<div class=\'k-loading-pdf-progress\'>').appendTo(loading).suixProgressBar({
                    type: 'chunk',
                    chunkCount: 10,
                    min: 0,
                    max: 1,
                    value: 0
                }).data('suixProgressBar');
                deferred.progress(function (e) {
                    pb.value(e.progress);
                }).always(function () {
                    suix.destroy(loading);
                    loading.remove();
                });
            };
        }
        ...
        function tableClick(e) {
            ...
            if (isInput && currentTarget.find(suix.roleSelector('filtercell')).length) {
            ...
            if (isHeader || !isInput) {
                setTimeout(function () {
                    if (!(isIE8 && $(suix._activeElement()).hasClass('k-widget'))) {
                        if ($(suix._activeElement()).is(CHECKBOXINPUT) || !isInputElement(suix._activeElement()) || !$.contains(currentTable, suix._activeElement())) {
                            focusTable(currentTable, true);
                        }
                    }
                });
            }
            if (isHeader && !suix.support.touch) {
                e.preventDefault();
            }
        }
        function leftMostPosition(element, rtl) {
            ...
            var result = 0;
            if (suix.support.browser.webkit) {
